<h1 align="center">Hi, I'm Abhishek Shrikant Nazare</h1>

<p align="center">
  Backend Engineer • Distributed Systems Enthusiast • MS CISE @ University of Florida
</p>

<p align="center">
  <strong>Graduating May 2027</strong> • Looking for <strong>2026 Co-op / Internship</strong> and <strong>2027 New Grad SDE roles</strong>
</p>

<p align="center">
  <a href="mailto:anazare@ufl.edu">
    <img src="https://img.shields.io/badge/Email-anazare%40ufl.edu-D14836?style=for-the-badge&logo=gmail&logoColor=white" alt="Email">
  </a>
  <a href="https://github.com/AbhishekNazare">
    <img src="https://img.shields.io/badge/GitHub-AbhishekNazare-181717?style=for-the-badge&logo=github" alt="GitHub">
  </a>
  <a href="https://www.linkedin.com/">
    <img src="https://img.shields.io/badge/LinkedIn-Connect-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn">
  </a>
</p>

<p align="center">
  <img src="https://img.shields.io/badge/Java-Spring%20Boot-6DB33F?style=for-the-badge&logo=springboot&logoColor=white" alt="Java Spring Boot">
  <img src="https://img.shields.io/badge/Distributed%20Systems-Kafka-231F20?style=for-the-badge&logo=apachekafka&logoColor=white" alt="Kafka">
  <img src="https://img.shields.io/badge/Backend-Microservices-0f172a?style=for-the-badge" alt="Backend Microservices">
  <img src="https://img.shields.io/badge/Docker-Infrastructure-2496ED?style=for-the-badge&logo=docker&logoColor=white" alt="Docker">
</p>

---

## About Me

I am a Master's student in Computer and Information Science at the University of Florida, graduating in **May 2027**.

I am currently looking for **2026 Software Engineering internships / co-ops** and **2027 New Grad SDE roles**, especially in backend engineering, distributed systems, infrastructure, platform engineering, databases, and reliability-focused software engineering.

I have **2.5+ years of software engineering experience** building scalable Java-based microservices and backend systems. My work has included API design, authentication, role-based access control, Redis caching, PostgreSQL-backed services, Kafka-based systems, production debugging, and maintainable backend architecture.

I enjoy building systems where correctness, performance, reliability, and clean engineering practices matter.

---

## What I Like Working On

• Backend services and microservices  
• Distributed systems and event-driven architectures  
• Databases, caching, and data access optimization  
• Concurrent programming and multithreaded systems  
• Authentication, authorization, and secure backend workflows  
• System design, scalability, and reliability engineering  
• Production debugging, observability, and root-cause analysis  

---

## Education

**University of Florida**  
Master of Science in Computer and Information Science  
**Aug 2025 – May 2027**  
Gainesville, FL

**PES University**  
Bachelor of Technology in Computer Science and Engineering  
**Aug 2019 – May 2023**  
Bengaluru, India

---

## Featured Projects

### RaftKV Studio  
**C++20 • Raft Consensus • Python • React • TypeScript • Docker**

An interactive distributed key-value store built around the Raft consensus protocol.

• Built a C++20 Raft-based distributed key-value store with leader election, quorum replication, follower catch-up, failover handling, persistent storage primitives, snapshots, and log compaction.  
• Designed a deterministic 3-node in-process Raft cluster test harness for safety scenarios including no-quorum rejection, committed-state preservation after failover, partition recovery, and snapshot installation.  
• Implemented a Python control-plane API and React/TypeScript observability dashboard for live consensus demos, topology visualization, KV commands, fault injection, and event timelines.  
• Added Dockerized backend tests, GitHub Actions CI, phase-based Git branches, clean commits, and documentation for interview-ready demos.  

<p align="center">
  <a href="https://github.com/AbhishekNazare/raftkv-studio">
    <img alt="RaftKV Studio repo card" src="https://github-readme-stats.vercel.app/api/pin/?username=AbhishekNazare&repo=raftkv-studio&theme=github_dark&hide_border=true">
  </a>
</p>

---

### Concurrent Task Processing System  
**Java • Multithreading • Thread Pools • Async Processing**

A backend processing system focused on concurrent execution, thread safety, and reliable task handling.

• Designed a multithreaded Java processing engine using thread pools and async task execution to parallelize financial report generation.  
• Ensured thread safety using synchronization mechanisms and idempotent task handling.  
• Prevented race conditions under concurrent workloads and improved reliability of backend task execution.  

---

### Event-Driven Microservices Platform  
**Kafka • Distributed Systems • Fault Tolerance • Microservices**

A distributed backend platform for real-time transaction processing and system notifications.

• Engineered a Kafka-based event-driven architecture for reliable inter-service communication.  
• Designed retry mechanisms, dead-letter queues, and fault-tolerant patterns to prevent state divergence under failure scenarios.  
• Focused on consistency, resilience, and scalable service-to-service communication.  

---



## Technical Skills

```text
Languages:              Java, Python, C, C++, SQL
Core CS:                Object-Oriented Programming, Data Structures, Algorithms, Design Patterns
Backend:                Spring Boot, FastAPI, REST API Design, Microservices, JWT, OAuth
Databases & Cache:      PostgreSQL, Redis
Distributed Systems:    Kafka, RabbitMQ, Event-Driven Architecture, Async Processing
Concurrency:            Multithreading, Thread Pools, Synchronization, Race Condition Prevention
Infrastructure:         Docker, Git, GitHub Actions, JIRA
Debugging:              JVM Debugging, Structured Logs, Production Issue Diagnosis